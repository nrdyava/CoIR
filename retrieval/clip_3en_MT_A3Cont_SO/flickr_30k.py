...
device = 4
exp_name = '2024-12-10-20-49-19-105939 + clip_inbatch_3en_MT_A3Cont_SO_LR_1e-5'
batch_size=64
...
from src.models.clip.inbatch_3en_MT_A3Cont_SO import CLIPModel
...
exp_results_i2t.to_csv(os.path.join(out_dir, exp_name, 'flickr-30k-[image-2-text]-experiment_results.csv'), index=False)
exp_results_t2i.to_csv(os.path.join(out_dir, exp_name, 'flickr-30k-[text-2-image]-experiment_results.csv'), index=False)
...
for checkpoint in checkpoints:
    ...
    with open(os.path.join(out_dir, exp_name, 'outputs'+'-flickr-30k-[image-2-text]-'+checkpoint+'.json'), "w") as json_file:
    ...
    with open(os.path.join(out_dir, exp_name, 'metrics'+'-flickr-30k-[image-2-text]-'+checkpoint+'.json'), "w") as json_file:
    ...
    with open(os.path.join(out_dir, exp_name, 'outputs'+'-flickr-30k-[text-2-image]-'+checkpoint+'.json'), "w") as json_file:
    ...
    with open(os.path.join(out_dir, exp_name, 'metrics'+'-flickr-30k-[text-2-image]-'+checkpoint+'.json'), "w") as json_file:
        json.dump(metrics, json_file, indent=4)
