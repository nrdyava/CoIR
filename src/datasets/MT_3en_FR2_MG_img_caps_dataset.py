...
class dataset(torch.utils.data.Dataset):
    def __init__(self, config, dataset_split):
        ...
        if dataset_split == 'train':
            lasco_json = json.load(open(os.path.join(self.lasco_data_path, 'metadata', 'lasco_train_indexed_MT(img_cap)_MG.json'), 'r'))
            self.triplets = list(
                map(
                    lambda x: {
                        ...
                        'query-text-forward-mg': x['coir']['query-text-forward-mg'],
                        'query-text-reverse-mg': x['coir']['query-text-reverse-mg']
                    },
                    lasco_json
                )
            )
            ...
            lasco_json = json.load(open(os.path.join(self.lasco_data_path, 'metadata', 'lasco_val_indexed_MT(img_cap)_MG.json'), 'r'))
            self.triplets = list(
                map(
                    lambda x: {
                        'query-image-id': x['coir']['query-image-id'],
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'query-text-for': triplet['query-text-forward-mg'] if (random.random() < self.p_mg and self.dataset_split == 'train') else triplet['query-text'],
            'query-text-rev': triplet['query-text-reverse-mg'],
    ...
    def collate_fn(self, batch):
        query_text_for = list(map(lambda x: x['query-text-for'], batch))
        query_text_for = self.tokenizer(query_text_for, padding=True, return_tensors="pt", max_length=77, truncation=True)
        
        query_text_rev = list(map(lambda x: x['query-text-rev'], batch))
        query_text_rev = self.tokenizer(query_text_rev, padding=True, return_tensors="pt", max_length=77, truncation=True)
        
        align_text = list(map(lambda x: x['align-text'], batch))
        align_text = self.tokenizer(align_text, padding=True, return_tensors="pt", max_length=77, truncation=True)
        ...
        return {
            ...
            'query-text-for': query_text_for,
            'query-text-rev': query_text_rev,
