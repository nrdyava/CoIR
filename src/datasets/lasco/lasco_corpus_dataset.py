...
class lasco_corpus_dataset_clip(Dataset):
    def __init__(self, dataset_split, lasco_data_path, clip_ckpt_path):
        ...
        if dataset_split == 'train':
            corpus = json.load(open(os.path.join(lasco_data_path, 'metadata', 'lasco_train_corpus.json'), 'r'))
            ...
            corpus = json.load(open(os.path.join(lasco_data_path, 'metadata', 'lasco_val_corpus.json'), 'r'))
            self.image_book = list(
                map(
                    lambda x: {
                        'image-key': x['image-id'], 
...
class lasco_corpus_dataset_flava(Dataset):
