...
class CLIPModel(L.LightningModule):
    def __init__(self, config):
        ...
        self.logit_scale_rev = torch.nn.Parameter(torch.ones([]) * np.log(1 / self.config['loss_fn']['temperature_instruct']))
        if not self.config['loss_fn']['train_temperature_instruct']:
            self.logit_scale_rev.requires_grad = False
    ...
    # For alignment not for instruction
    ...
    def img_txt_img_forward(self, batch):
        query_image = batch['query-image']
        query_text_for = batch['query-text-for']
        query_text_rev = batch['query-text-rev']
        target_image = batch['target-image']
        align_image = batch['align-image']
        align_text = batch['align-text']
        
        query_image_embeds = self.image_encoder(**query_image).image_embeds
        query_text_for_embeds = self.text_encoder_instruct(**query_text_for).text_embeds
        query_text_rev_embeds = self.text_encoder_instruct(**query_text_rev).text_embeds
        ...
        return {
            ...
            'query-text-for-embeds': query_text_for_embeds, 
            'query-text-rev-embeds': query_text_rev_embeds,
    ...
    def training_step(self, batch, batch_idx):
        ...
        query_text_for_embeds = outs['query-text-for-embeds']
        query_text_rev_embeds = outs['query-text-rev-embeds']
        ...
        target_hat_embeds = self.fuse_embeddings(query_image_embeds, query_text_for_embeds, dir = 'forward')
        target_hat_embeds = self.normalize_embeddings(target_hat_embeds)
        
        query_hat_embeds = self.fuse_embeddings(target_image_embeds, query_text_rev_embeds, dir = 'forward')
        ...
        if self.gather_embeddings and dist.is_initialized():
            ...
            query_image_embeds_concat = torch.cat([query_image_embeds, target_image_embeds], dim=0)
            ...
            loss_rev, avg_rank_rev, acc_rev = self.loss_fn_instruct(query_hat_embeds, query_image_embeds_concat, self.logit_scale_rev, gpu_id)
            
            loss_align = self.loss_fn_align(align_image_embeds, align_text_embeds, self.logit_scale_align)
            loss = (loss_for + loss_rev + loss_align)/3
        ...
        self.manual_backward((loss_for + loss_rev)/2)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_loss_rev', loss_rev, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        ...
        self.log('val_avg_rank_rev', avg_rank_rev.detach().cpu().numpy().item(), on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        self.log('val_acc_rev', acc_rev.detach().cpu().numpy().item(), on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
