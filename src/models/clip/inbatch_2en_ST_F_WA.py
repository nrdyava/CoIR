...
class CLIPModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('w_img', weights[0].detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
        self.log('w_txt', weights[1].detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
