...
class CLIPModel(L.LightningModule):
    def __init__(self, config):
        ...
        self.logit_scale_for = torch.nn.Parameter(torch.ones([]) * np.log(1 / self.config['loss_fn']['temperature']))
        self.logit_scale_bac = torch.nn.Parameter(torch.ones([]) * np.log(1 / self.config['loss_fn']['temperature']))
        if not self.config['loss_fn']['train_temperature']:
            self.logit_scale_for.requires_grad = False
            self.logit_scale_bac.requires_grad = False
    ...
    def training_step(self, batch, batch_idx):
        ...
        query_hat_embeds = self.fuse_embeddings(target_image_embeds, -query_text_embeds)
        ...
        if self.gather_embeddings and dist.is_initialized():
            ...
            gpu_id = 0
            target_image_embeds = torch.cat([target_image_embeds, query_image_embeds], dim=0)
            query_image_embeds = torch.cat([query_image_embeds, target_image_embeds], dim=0)
            
            loss_for, avg_rank_for, acc_for = self.loss_fn(target_hat_embeds, target_image_embeds, self.logit_scale_for, gpu_id)
            loss_bac, avg_rank_bac, acc_bac = self.loss_fn(query_hat_embeds, query_image_embeds, self.logit_scale_bac, gpu_id)
            loss = (loss_for + loss_bac)/2
        ...
        self.log('train_loss_bac', loss_bac, on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        ...
        self.log('train_avg_rank_bac', avg_rank_bac.detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        ...
        self.log('train_acc_bac', acc_bac.detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        ...
        self.log('logit_scale_bac', self.logit_scale_bac.exp().detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_loss_bac', loss_bac, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        ...
        self.log('val_avg_rank_bac', avg_rank_bac.detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
        ...
        self.log('val_acc_bac', acc_bac.detach().cpu().numpy().item(), on_step=True, on_epoch=True, prog_bar=False, logger=True, sync_dist=True, batch_size = batch_size)
